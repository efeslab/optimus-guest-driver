...
*/
DLMALLOC_EXPORT mspace create_mspace(size_t capacity, int locked, struct _fpga_handle *fpga);
...
static void* sys_alloc(mstate m, size_t nb) {
  ...
  */
  ...
  if (HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
    assert(is_initialized(m) && "mspace should not init twice, see create_mspace");
    if (!is_initialized(m)) { //initialize large 64GB mmap
        ...
        if (m->seg.base == MAP_FAILED) {
            ...
            vai_afu_set_mem_base(m->fpga, (uint64_t)m->seg.base);
    }
    ...
    int ret_vmap;
    if (ret_mp != -1)
        ret_vmap = vai_afu_map_region(m->fpga, (uint64_t)mp, asize);
    else
        ret_vmap = -1;
    if (ret_vmap != -1) {
      tbase = mp;
        ...
        if (is_mmapped_segment(sp)) {
          if (HAVE_MMAP &&
            ...
            if ((mprotect(sp->base + newsize, extra, PROT_NONE) != -1) &&
                ...
                (vai_afu_unmap_region(m->fpga, (uint64_t)(sp->base + newsize), extra) != -1)) {
              released = extra;
            }
          }
        }
...
mspace create_mspace(size_t capacity, int locked, struct _fpga_handle *fpga) {
  mstate m = 0;
  size_t msize;
  ensure_initialization();
  ...
  if (capacity < (size_t) -(msize + TOP_FOOT_SIZE + mparams.page_size)) {
    ...
    int ret_vmap;
    if (ret_mpt != -1) {
        vai_afu_set_mem_base(fpga, (uint64_t)tbase);
        ret_vmap = vai_afu_map_region(fpga, (uint64_t)tbase, tsize);
    }
    else
        ret_vmap = -1;
